...
namespace MoreMountains.NiceVibrations
{
    ...
    public class MMNVBuildPostProcessor : IPostprocessBuildWithReport
    {
        ...
        private static void WritePropertiesToFramework(PBXProject pbxProject, string targetGUID, string pbxProjectPath, string pluginPath, bool forceAlwaysEmbedSwiftStandardLibraries)
        {
            ...
            foreach (var framework in new[] {targetGUID, pbxProject.GetUnityFrameworkTargetGuid()})
            {
                pbxProject.SetBuildProperty(framework, "ENABLE_BITCODE", "NO");
                pbxProject.SetBuildProperty(framework, "EMBEDDED_CONTENT_CONTAINS_SWIFT", "YES");
                pbxProject.SetBuildProperty(framework, "ALWAYS_EMBED_SWIFT_STANDARD_LIBRARIES", "NO");
                pbxProject.SetBuildProperty(framework, "SWIFT_VERSION", "5.0");
                pbxProject.SetBuildProperty(framework, "NSAllowsArbitraryLoads", "YES");
            }
